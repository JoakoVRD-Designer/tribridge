...
const EXIT = { ok: 0, usage: 1, failed: 2, empty: 3, quota: 10, auth: 11, timeout: 12, missing: 13, recursion: 14, denied: 15, blocked: 16 };
...
const HINT = {
  ...
  blocked: 'your own sandbox blocked launching the other agent (it needs to run a program outside the workspace, with network). '
    + 'Run this same tribridge command again outside the sandbox — in Codex, request escalated permissions / approval for it.',
...
const TRANSIENT = /\bEOF\b|ECONNRESET|connection reset|TLS handshake timeout|i\/o timeout|dial tcp|\b50[234]\b|service unavailable/i;
...
async function delegate(req, cfg) {
  ...
  const warnings = [];
  // Measured: Antigravity's terminal starts in ~/.gemini/antigravity-cli/scratch, so a
  // `--dir .` typed inside agy points there instead of at the user's project.
  if (dir && /[\\/]\.gemini[\\/]antigravity-cli[\\/]scratch(?:[\\/]|$)/i.test(dir)) {
    warnings.push(`--dir is Antigravity's scratch folder (${dir}), not a project. Pass the project's absolute path to --dir.`);
  }
  ...
  let after = gitSnapshot(dir);
  // One retry for a transient network drop (measured: agy's sign-in check failing with
  // "…userinfo: EOF"), but only when nothing was produced and nothing changed on disk.
  if (!res.spawnError && !res.timedOut && res.code !== 0 && TRANSIENT.test(`${res.stdout}\n${res.stderr}`)) {
    const changed = diffSnapshots(before, after);
    if (!changed || changed.length === 0) {
      warnings.push('transient network error; retried once.');
      res = await launch();
      after = gitSnapshot(dir);
    }
  }

  let out;
  if (res.spawnError) {
    // EPERM/EACCES: the binary exists but the *host's* sandbox won't let us launch it
    // (measured: Codex's workspace-write sandbox on Windows).
    const blocked = ['EPERM', 'EACCES'].includes(res.spawnError.code);
    out = { text: '', kind: blocked ? 'blocked' : 'missing', detail: String(res.spawnError), usage: {}, warnings: [] };
  } else {
