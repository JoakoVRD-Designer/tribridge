---
description: Delegate a task to Codex or Antigravity (Gemini) via tribridge, then verify the result.
argument-hint: "--to codex|agy [--tier fast|balanced|deep] [--mode read|write] <task>"
---

Delegate this task with the `tribridge` skill: $ARGUMENTS

1. If no `--to` was given, pick the agent that fits (agy: bulk work, web search, audio/video, huge
   files · codex: precise implementation, debugging). If the task is small, say so and do it yourself.
