...
description: |
  Hands one well-specified, bulky unit of work to another AI agent (Claude Code, Codex or
  Antigravity/Gemini — never the one running it) through tribridge and returns the other agent's digest. Use it proactively for above-break-even
  work — scaffolding, exhaustive test generation, mechanical migrations, long-context reading
  distilled to a digest, web research via Gemini — so the bulk is not generated by the calling model. It does not verify and never claims success: the caller verifies.
...
limited to `tribridge delegate …`, `tribridge job …` and `tribridge host` — you cannot write files yourself, and
...
2. Run `tribridge host` (never delegate to yourself), then choose `--to`: `agy` for bulk/cheap work,
   web search, huge inputs, audio/video; `codex` for precise implementation and debugging; `claude`
   for architecture and careful refactors. Use `--dir <repo root>`, and `--mode write` if it must edit files.
